...
class Player(pygame.sprite.Sprite):
    ...
    # Không thay đổi từ đây đến cmt sau
    def shoot(self):
        now = pygame.time.get_ticks()
    ...
    def apply_powerup(self, ptype):
        now = pygame.time.get_ticks()
        if ptype == "heal":
            heal = max(1, PLAYER_MAX_HP // 4)
            self.hp = min(self.hp + heal, PLAYER_MAX_HP)
        elif ptype == "triple":
...
class Boss(pygame.sprite.Sprite):
    ...
    def dhp_bar(self, surface, ox=0, oy=0):
        bw, bh = 240, 12
        x = WIDTH // 2 - bw // 2 + ox
        y = 44 + oy
...
def dhud(surface, player, score, high_score, combo,
    ...
    # Hearts
    pygame.draw.rect(surface, (30, 30, 30),
                     (8, 8, PLAYER_MAX_HP * 22 + 10, 26), border_radius=4)
                ...
                if boss and b.rect.colliderect(boss.rect) and not boss.entering:
                    ...
                    if dead:
                        ...
                        if is_campaign and boss_count >= BOSSES_PER_LEVEL:
                            ...
                            boss_count = 0
                            last_boss_died_ms = now
                            player.hp = min(player.hp + 2, PLAYER_MAX_HP)
                    continue
                killed = False
